...
#if FXAA_GLSL_120
    #define FxaaTexLod0(tex, pos) \
        texture2DLod((tex), (pos).xy, 0.0)
#endif
#if FXAA_GLSL_130
    #define FxaaTexLod0(tex, pos) \
        textureLod((tex), (pos).xy, 0.0)
#endif
#if FXAA_HLSL_3
    #define FxaaTexLod0(tex, pos) \
        tex2Dlod((tex), float4((pos).xy, 0.0, 0.0))
#endif
#if FXAA_HLSL_4
    #define FxaaTexLod0(tex, pos) \
        tex.tex.SampleLevel((tex).smpl, (pos).xy, 0.0)
#endif

/*--------------------------------------------------------------------------*/

#if FXAA_GLSL_120
    #define FxaaTexGrad(tex, pos, grad) \
        texture2DGrad((tex), (pos).xy, (grad), (grad))
#endif
#if FXAA_GLSL_130
    #define FxaaTexGrad(tex, pos, grad) \
        textureGrad((tex), (pos).xy, (grad), (grad))
#endif
#if FXAA_HLSL_3
    #define FxaaTexGrad(tex, pos, grad) \
        tex2Dgrad((tex), (pos).xy, (grad), (grad))
#endif
#if FXAA_HLSL_4
    #define FxaaTexGrad(tex, pos, grad) \
        tex.tex.SampleGrad((tex).smpl, (pos).xy, (grad), (grad))
#endif

/*--------------------------------------------------------------------------*/

#if FXAA_GLSL_120
    #define FxaaTexOff(tex, pos, off, rcpFrame) \
        texture2DLodOffset((tex), (pos).xy, 0.0, (off).xy)
#endif
#if FXAA_GLSL_130
    #define FxaaTexOff(tex, pos, off, rcpFrame) \
        textureLodOffset((tex), (pos).xy, 0.0, (off).xy)
#endif
#if FXAA_HLSL_3
    #define FxaaTexOff(tex, pos, off, rcpFrame) \
        tex2Dlod((tex), float4((pos).xy + ((off) * (rcpFrame)), 0, 0))
#endif
#if FXAA_HLSL_4
    #define FxaaTexOff(tex, pos, off, rcpFrame) \
        tex.tex.SampleLevel((tex).smpl, (pos).xy, 0.0, (off).xy)
#endif
...
float3 FxaaFilterReturn(float3 rgb)
{
    ...
    return FxaaSel3(rgb * FxaaToFloat3(1.0 / 12.92),
                    FxaaPow3(rgb * FxaaToFloat3(1.0 / 1.055) + FxaaToFloat3(0.055 / 1.055), FxaaToFloat3(2.4)),
...
//
// pos:  Output of FxaaVertexShader interpolated across screen.
//       xy -> actual texture position {0.0 to 1.0}
//
// rcpFrame:  SHOULD BE PIXEL SHADER CONSTANTS!!!
